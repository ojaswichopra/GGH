...
int evaluateExpression(const Circuit& circuit, const string& node,vector<int>& inputVector) {
    if (circuit.find(node) != circuit.end()) {
        string expression = circuit.at(node);

        if (expression[0] == '~') {
            // NOT gate
            int result = evaluateExpression(circuit, expression.substr(1),inputVector);
            return !result;
            ...
            int result1 = evaluateExpression(circuit, operand1,inputVector);
            int result2 = evaluateExpression(circuit, operand2,inputVector);
    ...
    else 
    {
        /* code */
        if (node == "A")
                return inputVector[0];  // Input A is 1
            else if (node == "B")
                return inputVector[1];  // Input B is 0
            else if (node == "C")
                return inputVector[2];  // Input C is 1
            else if (node == "D")
                return inputVector[3];  // Input D is 1
    }
...
int evaluateCircuit(const Circuit& circuit,vector<int>& inputVector) {
    string outputNode = "Z";
    return evaluateExpression(circuit, outputNode,inputVector);
}
...
int main() {
    ...
    string faultNode;
    string faultType;
    // cout<<"Enter Fault Characteristics"<< endl;
